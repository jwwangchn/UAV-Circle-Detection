...
pair<vector<vector<double>>, vector<Vec3f>> circleDetection(Mat img)
{
	...
	for (int i = 0; i < img.cols; i++)
	{
		for (int j = 0; j < img.rows; j++)
		{
			...
			if (Th1_Red > R_G_R_RED_CIRCLE && Th2_Red > R_B_R_RED_CIRCLE &&RedChannel.at<uchar>(j, i)>190) //|| ((Th1_Yellow > R_B_R_YELLOW_CIRCLE && Th2_Yellow > G_B_G_YELLOW_CIRCLE)) || (Th1_Blue > B_R_B_BLUE_CIRCLE && Th2_Blue > B_G_B_BLUE_CIRCLE))
			{
